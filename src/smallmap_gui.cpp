...
{
	static const int tile_zoomlevels[] = { 1, 1, 1, 1, 1, 2, 4, 6, 8 };  // Available zoom-out levels. Bigger number means more world tiles per map tile.
	static const int ui_zoomlevels[]   = { 16, 8, 4, 2, 1, 1, 1, 1, 1 }; // Available zoom-in levels. Bigger number means more pixels drawn per map tile.
...
void SmallMapWindow::DrawVehicles(const DrawPixelInfo *dpi, Blitter *blitter) const
{
	for (const Vehicle *v : Vehicle::Iterate()) {
		...
		int min_i = std::max(0, -y);
		int max_i = std::min(this->ui_zoom, dpi->height - y);
		int min_j = std::max(0, -x);
		int max_j = std::min(2 * this->ui_zoom, dpi->width - x);
...
void SmallMapWindow::DrawSmallMap(DrawPixelInfo *dpi) const
{
	...
	/* Where is this tile located within the minimap? */
